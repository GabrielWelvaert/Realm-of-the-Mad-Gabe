...
void RenderTextSystem::Update(SDL_Renderer* renderer, std::unique_ptr<AssetStore>& assetStore, const SDL_Rect& camera, std::unique_ptr<Registry>& registry){
    for (auto& entity: GetSystemEntities()){
        ...
        if(textlabel.spawnframe){
            ...
            // std::cout << textlabel.text << ": " << textlabel.textwidth << ", " << textlabel.textheight << std::endl;
        ...
        if(!textlabel.isFixed){
            ...
            position.x = parentPosition.x + (static_cast<int>(victimSprite.width) * 6 /2 - textlabel.textwidth/2); // used to be victimSprite.height? why
            if(position.x - camera.x >= 750){continue;}
            if(textlabel.spawnframe){
